...
você deverá imprimir todo o vetor e utilizá-lo novamente para os próximos números que forem lidos. Terminada 
a leitura, deve-se imprimir o conteúdo que restou em cada um dos dois vetores, imprimindo primeiro os valores 
...
for num in range(15): 
    ...
    if len(impar) == 5:
        for i, p in enumerate(impar):
            print(f'impar[{i}] = {p}')
        impar.clear()
    if len(par) == 5: 
        for io, po in enumerate(par):
            print(f'par[{io}] = {po}')
        par.clear()
...
for f, g in enumerate(impar):
    print(f'impar[{f}] = {g}') 
for h, j in enumerate(par):
    print(f'par[{h}] = {j}')